...
hotel_id_code_df = train_csv.drop(columns=["image_id"]).drop_duplicates().reset_index(drop=True)
...
def train_and_validate(dataframe):
    val_df = dataframe.groupby("hotel_id").sample(1, random_state=60)
    train_data = dataframe[~dataframe["image_id"].isin(val_df["image_id"])]
    ...
    valid_loader = DataLoader(val_dataset, num_workers=0, batch_size=64, shuffle=False)
